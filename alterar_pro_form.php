<?php 
    require "alterar_pro_p.php";
    include "conexao/conectar.php";
    $id=$_SESSION['id'];
    $buscar=(mysqli_query($conectar, "select * from produto where id='$id';"));
    $linha=(mysqli_fetch_array($buscar));
    $descricao=htmlspecialchars($linha[1]);
    $valor=htmlspecialchars($linha[2]);
?>
<!DOCTYPE html>
<html lang="en">
        ...
        <div class="col-xl-6 col-lg-8">
          ...
          <br>
            <form action='' method='get'>
              ...
              <input type="text" required="" name='valor' value='<?= $valor ?>' />
              <label>Valor por peso:</label>
              </div>
              <p>Descrição:</p>
              <textarea rows="10" cols="40" maxlength="500" name='descricao'><?= $descricao?></textarea><p><p>
              <br>
...
<?php
      ...
      if (isset($_GET['cadastro'])){
        ...
        $valorf = $_GET['valor'];
        $descricaof = $_GET['descricao'];
    
        $pro=mysqli_query($conectar, "select * from produto where id='$id';");
    
          if(mysqli_num_rows($pro) != 0){
            $inserir=mysqli_query($conectar, "update produto set descricao='$descricaof', valor_peso='$valorf' where id='$id';");
            unset($_SESSION['id']);
          }
          else{
