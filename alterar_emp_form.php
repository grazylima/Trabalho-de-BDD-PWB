<?php 
    require "alterar_emp_p.php";
    include "conexao/conectar.php";
    $id=$_SESSION['id'];
    $buscar=(mysqli_query($conectar, "select * from empresa where id='$id';"));
    $linha=(mysqli_fetch_array($buscar));
    $nome=htmlspecialchars($linha[1]);
    $endereco=htmlspecialchars($linha[2]);
    $numero=htmlspecialchars($linha[3]);
    $bairro=htmlspecialchars($linha[4]);
    $cidade=htmlspecialchars($linha[5]);
    $telefone=htmlspecialchars($linha[6]);
?>
<!DOCTYPE html>
<html lang="en">
        ...
        <div class="col-xl-6 col-lg-8">
          ...
          <br>
            <form action='' method='get'>
              ...
              <div class="input-container">		
              <input type="text" required="" name='endereco' value='<?= $endereco ?>' />
              <label>Endereço:</label>
              </div>

              <div class="input-container">		
              <input type="number" required="" name='numero' value='<?= $numero ?>' />
              <label>Número:</label>
              </div>

              <div class="input-container">		
              <input type="text" required="" name='bairro' value='<?= $bairro ?>' />
              <label>Bairro:</label>
              </div>

              <div class="input-container">		
              <input type="text" required="" name='cidade' value='<?= $cidade ?>' />
              <label>Cidade:</label>
              </div>
...
<?php
      ...
      if (isset($_GET['cadastro'])){
        ...
        $nomef = $_GET['nome'];
        $enderecof = $_GET['endereco'];
        $numerof = $_GET['numero'];
        $bairrof= $_GET['bairro'];
        $cidadef = $_GET['cidade'];
        $telefonef = $_GET['telefone'];
    
        $emp=mysqli_query($conectar, "select * from empresa where id='$id';");
    
          if(mysqli_num_rows($emp) != 0){
            $inserir=mysqli_query($conectar, "update empresa set nome='$nomef', endereco='$enderecof', numero='$numerof', 
            bairro='$bairrof', cidade='$cidadef', telefone='$telefonef' where id='$id';");
            unset($_SESSION['id']);
          }
          else{
